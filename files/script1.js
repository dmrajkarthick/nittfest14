...
function load2(){
    ...
    $(".ajaxSubevent").click(function(){
        ...
        $.ajax("assets/serve.php?q=s_"+$(this).attr("name")+"&c="+gh).done(function(html){
            ...
            $("#n-content-inner").css({top:"0%"}).fadeOut('fast').attr("class",html.language).html(html.content).fadeIn('fast',attachMouse);
            $(".tab0").fadeIn("fast");
...
$(document).ready(function(){
    if(!!(window.history && history.pushState))
        $('.historye').click(function(){console.log(pushallow) ;if(pushallow==1) {history.pushState(null,null,$(this).attr('href') ); }});
